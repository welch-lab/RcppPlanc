...
class GNSYMNMF : public NMF<T> {
 ...
 public:
  GNSYMNMF(const T &A, int lowrank, const unsigned int ncores = 0) : NMF<T>(A, lowrank) {
  ...
  GNSYMNMF(const T &A, const arma::mat &llf, const arma::mat &rlf, const unsigned int ncores = 0) : NMF<T>(A, llf, rlf) {
    this->W.clear();  //  clear W variable
    allocateMatrices();
    sqnormA = this->normA * this->normA;
